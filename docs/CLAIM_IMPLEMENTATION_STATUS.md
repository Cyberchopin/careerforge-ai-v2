...
| Claim provenance / ProofGraph | `lib/analyzer.ts` links generated bullets to resume excerpts and constructs a deterministic graph; candidate-supplied URLs are stored by `components/CareerForge.tsx`. External URLs are not checked for content or ownership. | “Source-linked bullets and candidate-supplied artifact visualization,” not “ProofGraph verifies claims.” |
